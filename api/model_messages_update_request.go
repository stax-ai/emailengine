/*
EmailEngine API

EmailEngine provides a RESTful API for managing email accounts, sending messages, and processing email data across multiple providers.  <h3>Authentication</h3> All API requests require authentication using an Access Token. You can generate and manage your tokens from the <a href=\"/admin/tokens\" target=\"_parent\"><strong>Access Tokens</strong></a> page.  Include your token in requests using one of these methods: - Query parameter: <code>?access_token=YOUR_TOKEN</code> - Authorization header: <code>Authorization: Bearer YOUR_TOKEN</code>  <h3>Request Processing</h3>  <strong>Sequential Processing:</strong> Requests to the same email account are processed sequentially to maintain data consistency. Multiple simultaneous requests will be queued.  <strong>Timeouts:</strong> Long-running operations may cause queued requests to timeout. Configure appropriate timeout values using the <code>X-EE-Timeout</code> header (in milliseconds).  <h3>Getting Started</h3> 1. <a href=\"/admin/tokens\" target=\"_parent\">Generate an Access Token</a> 2. <a href=\"/admin/accounts\" target=\"_parent\">Add an email account</a> 3. Start making API requests using the endpoints below

API version: 2.61.1
Contact: support@emailengine.app
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package api

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the MessagesUpdateRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &MessagesUpdateRequest{}

// MessagesUpdateRequest struct for MessagesUpdateRequest
type MessagesUpdateRequest struct {
	Search SearchQuery `json:"search"`
	Update *MessageUpdate `json:"update,omitempty"`
}

type _MessagesUpdateRequest MessagesUpdateRequest

// NewMessagesUpdateRequest instantiates a new MessagesUpdateRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewMessagesUpdateRequest(search SearchQuery) *MessagesUpdateRequest {
	this := MessagesUpdateRequest{}
	this.Search = search
	return &this
}

// NewMessagesUpdateRequestWithDefaults instantiates a new MessagesUpdateRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewMessagesUpdateRequestWithDefaults() *MessagesUpdateRequest {
	this := MessagesUpdateRequest{}
	return &this
}

// GetSearch returns the Search field value
func (o *MessagesUpdateRequest) GetSearch() SearchQuery {
	if o == nil {
		var ret SearchQuery
		return ret
	}

	return o.Search
}

// GetSearchOk returns a tuple with the Search field value
// and a boolean to check if the value has been set.
func (o *MessagesUpdateRequest) GetSearchOk() (*SearchQuery, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Search, true
}

// SetSearch sets field value
func (o *MessagesUpdateRequest) SetSearch(v SearchQuery) {
	o.Search = v
}

// GetUpdate returns the Update field value if set, zero value otherwise.
func (o *MessagesUpdateRequest) GetUpdate() MessageUpdate {
	if o == nil || IsNil(o.Update) {
		var ret MessageUpdate
		return ret
	}
	return *o.Update
}

// GetUpdateOk returns a tuple with the Update field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MessagesUpdateRequest) GetUpdateOk() (*MessageUpdate, bool) {
	if o == nil || IsNil(o.Update) {
		return nil, false
	}
	return o.Update, true
}

// HasUpdate returns a boolean if a field has been set.
func (o *MessagesUpdateRequest) HasUpdate() bool {
	if o != nil && !IsNil(o.Update) {
		return true
	}

	return false
}

// SetUpdate gets a reference to the given MessageUpdate and assigns it to the Update field.
func (o *MessagesUpdateRequest) SetUpdate(v MessageUpdate) {
	o.Update = &v
}

func (o MessagesUpdateRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o MessagesUpdateRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["search"] = o.Search
	if !IsNil(o.Update) {
		toSerialize["update"] = o.Update
	}
	return toSerialize, nil
}

func (o *MessagesUpdateRequest) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"search",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varMessagesUpdateRequest := _MessagesUpdateRequest{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varMessagesUpdateRequest)

	if err != nil {
		return err
	}

	*o = MessagesUpdateRequest(varMessagesUpdateRequest)

	return err
}

type NullableMessagesUpdateRequest struct {
	value *MessagesUpdateRequest
	isSet bool
}

func (v NullableMessagesUpdateRequest) Get() *MessagesUpdateRequest {
	return v.value
}

func (v *NullableMessagesUpdateRequest) Set(val *MessagesUpdateRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableMessagesUpdateRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableMessagesUpdateRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableMessagesUpdateRequest(val *MessagesUpdateRequest) *NullableMessagesUpdateRequest {
	return &NullableMessagesUpdateRequest{value: val, isSet: true}
}

func (v NullableMessagesUpdateRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableMessagesUpdateRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


