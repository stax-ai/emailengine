...
const (
	...
	QUEUE_DOCUMENTS Queue = "documents"
)

// All allowed values of Queue enum
var AllowedQueueEnumValues = []Queue{
	"notify",
	"submit",
	"documents",
}

func (v *Queue) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := Queue(value)
	for _, existing := range AllowedQueueEnumValues {
	...
	return fmt.Errorf("%+v is not a valid Queue", value)
}

// NewQueueFromValue returns a pointer to a valid Queue
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewQueueFromValue(v string) (*Queue, error) {
	ev := Queue(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for Queue: valid values are %v", v, AllowedQueueEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v Queue) IsValid() bool {
	for _, existing := range AllowedQueueEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to queue value
func (v Queue) Ptr() *Queue {
	return &v
}
