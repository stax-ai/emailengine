/*
EmailEngine API

EmailEngine provides a RESTful API for managing email accounts, sending messages, and processing email data across multiple providers.  <h3>Authentication</h3> All API requests require authentication using an Access Token. You can generate and manage your tokens from the <a href=\"/admin/tokens\" target=\"_parent\"><strong>Access Tokens</strong></a> page.  Include your token in requests using one of these methods: - Query parameter: <code>?access_token=YOUR_TOKEN</code> - Authorization header: <code>Authorization: Bearer YOUR_TOKEN</code>  <h3>Request Processing</h3>  <strong>Sequential Processing:</strong> Requests to the same email account are processed sequentially to maintain data consistency. Multiple simultaneous requests will be queued.  <strong>Timeouts:</strong> Long-running operations may cause queued requests to timeout. Configure appropriate timeout values using the <code>X-EE-Timeout</code> header (in milliseconds).  <h3>Getting Started</h3> 1. <a href=\"/admin/tokens\" target=\"_parent\">Generate an Access Token</a> 2. <a href=\"/admin/accounts\" target=\"_parent\">Add an email account</a> 3. Start making API requests using the endpoints below

API version: 2.61.1
Contact: support@emailengine.app
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package api

import (
	"encoding/json"
)

// checks if the Model28 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &Model28{}

// Model28 struct for Model28
type Model28 struct {
	Add []string `json:"add,omitempty"`
	Delete []string `json:"delete,omitempty"`
	Set []string `json:"set,omitempty"`
}

// NewModel28 instantiates a new Model28 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewModel28() *Model28 {
	this := Model28{}
	return &this
}

// NewModel28WithDefaults instantiates a new Model28 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewModel28WithDefaults() *Model28 {
	this := Model28{}
	return &this
}

// GetAdd returns the Add field value if set, zero value otherwise.
func (o *Model28) GetAdd() []string {
	if o == nil || IsNil(o.Add) {
		var ret []string
		return ret
	}
	return o.Add
}

// GetAddOk returns a tuple with the Add field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Model28) GetAddOk() ([]string, bool) {
	if o == nil || IsNil(o.Add) {
		return nil, false
	}
	return o.Add, true
}

// HasAdd returns a boolean if a field has been set.
func (o *Model28) HasAdd() bool {
	if o != nil && !IsNil(o.Add) {
		return true
	}

	return false
}

// SetAdd gets a reference to the given []string and assigns it to the Add field.
func (o *Model28) SetAdd(v []string) {
	o.Add = v
}

// GetDelete returns the Delete field value if set, zero value otherwise.
func (o *Model28) GetDelete() []string {
	if o == nil || IsNil(o.Delete) {
		var ret []string
		return ret
	}
	return o.Delete
}

// GetDeleteOk returns a tuple with the Delete field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Model28) GetDeleteOk() ([]string, bool) {
	if o == nil || IsNil(o.Delete) {
		return nil, false
	}
	return o.Delete, true
}

// HasDelete returns a boolean if a field has been set.
func (o *Model28) HasDelete() bool {
	if o != nil && !IsNil(o.Delete) {
		return true
	}

	return false
}

// SetDelete gets a reference to the given []string and assigns it to the Delete field.
func (o *Model28) SetDelete(v []string) {
	o.Delete = v
}

// GetSet returns the Set field value if set, zero value otherwise.
func (o *Model28) GetSet() []string {
	if o == nil || IsNil(o.Set) {
		var ret []string
		return ret
	}
	return o.Set
}

// GetSetOk returns a tuple with the Set field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Model28) GetSetOk() ([]string, bool) {
	if o == nil || IsNil(o.Set) {
		return nil, false
	}
	return o.Set, true
}

// HasSet returns a boolean if a field has been set.
func (o *Model28) HasSet() bool {
	if o != nil && !IsNil(o.Set) {
		return true
	}

	return false
}

// SetSet gets a reference to the given []string and assigns it to the Set field.
func (o *Model28) SetSet(v []string) {
	o.Set = v
}

func (o Model28) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o Model28) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Add) {
		toSerialize["add"] = o.Add
	}
	if !IsNil(o.Delete) {
		toSerialize["delete"] = o.Delete
	}
	if !IsNil(o.Set) {
		toSerialize["set"] = o.Set
	}
	return toSerialize, nil
}

type NullableModel28 struct {
	value *Model28
	isSet bool
}

func (v NullableModel28) Get() *Model28 {
	return v.value
}

func (v *NullableModel28) Set(val *Model28) {
	v.value = val
	v.isSet = true
}

func (v NullableModel28) IsSet() bool {
	return v.isSet
}

func (v *NullableModel28) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableModel28(val *Model28) *NullableModel28 {
	return &NullableModel28{value: val, isSet: true}
}

func (v NullableModel28) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableModel28) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


